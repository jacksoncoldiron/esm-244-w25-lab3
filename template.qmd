...
3. Develop a hypothesis
    ...
    Methodology: map out the plan, get psuedocode

    Clean data / ID variables
...
5. Data split into testing and training
...
7. Finalize the model (aka train on whole dataset)

8. Interpret results / visualize / communicate 
...
Cabin missing a lot of data - change to yes or no cabin
...
sib_sp: siblings or spouse
parch: parents and children
pclass: class


Variables of interest: age, sex, sib_sp, parch, pclass
Variables we don't need: passenger_id, ticket, names
Unsure: cabin, embarked

Insights
- fare could be collinear with pclass
- lots of na in age, sibling and parents
...
surv_df <- t_df |>
  mutate(survived = factor(survived),           ### categorical outcome variables need to factors
         pclass = factor(pclass)) |>            ### categorical outcome variables need to factors
  select(-cabin, -ticket)
```
Variables to include: class, age, sex, embark
...
lasso_rec<-recipe(survived~.,data=surv_train_df) %>%
  update_role(passenger_id, new_role = "ID") %>%       # role is not a predictor, but just a market, don't run
  step_rm(name,age) %>%                                # don't consider name and age because too many NA
  step_unknown(all_nominal(),-all_outcomes()) |>       # if you see NA, make a new factor class called unknown
  step_dummy(all_nominal(),-all_outcomes()) |>         # category is recognized as dummy
  step_zv(all_numeric(),-all_outcomes()) |>            # zero variance 
  step_normalize(all_numeric(),-all_outcomes())        # all numeric data normalized
